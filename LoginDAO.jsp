...
<%
    ...
    try {
        Class.forName("com.mysql.cj.jdbc.Driver");			// JDBC Driver 로드
        ...
        pstmt.setString(1, userID);			// PreparedStatement에 값 설정
        ...
        if (rs.next()) {
            ...
            if (password.equals(userPwd)) {
                // 비밀번호 일치
                session.setAttribute("userID", userID);			// 메인 페이지에 세션부여
...
%>
                <script>
                    alert("비밀번호가 잘못되었습니다.");
                    window.location.href = "test.jsp";			//test.jsp 이건 제가 테스트 할라고 만든 파일이고 이건 나중에 수정하면 됨
                </script>
<%
            }
        } else {
